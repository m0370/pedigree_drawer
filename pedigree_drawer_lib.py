...
- gender: "M" (□), "F" (○), "U" (◇)
- status: "affected" (filled), "deceased" (slash "/"), "stillbirth" (triangle + slash + SB),
...
class PedigreeChart:
    ...
    def _ordered_partners(self, p1_id: str, p2_id: str) -> Tuple[str, str]:
        p1 = self.people[p1_id]
        p2 = self.people[p2_id]
        if p1.gender == "M" and p2.gender == "F":
            return (p1_id, p2_id)
        if p1.gender == "F" and p2.gender == "M":
            return (p2_id, p1_id)
        return (p1_id, p2_id)
    ...
    def _draw_children_lines(self, parent: ET.Element, parent1: Person, parent2: Person, children: List[Person]) -> None:
        ...
        parent_bottom = parent1.y + self.symbol_size / 2
    ...
    def _draw_person(self, parent: ET.Element, person: Person) -> None:
        ...
        if any(st in person.status for st in ("miscarriage", "abortion", "stillbirth")):
            ...
            if "abortion" in person.status:
                add_line(cx - half * 0.8, cy - half * 0.8, cx + half * 0.8, cy + half * 0.8)  # "\" (distinct from deceased)
        ...
        add_text(cx, cy - half - 10, person.id, size=10, klass="txt id")

        below: List[str] = []
        if person.age and all(st not in person.status for st in ("miscarriage", "abortion", "pregnancy", "stillbirth")):
            below.append(f"d. {person.age}" if "deceased" in person.status else person.age)
        if person.name:
