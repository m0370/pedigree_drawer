...
*   `pedigree_drawer_lib.py`: GPTs の "Knowledge" にアップロードする描画エンジン (Python, v0.5)。
*   `render_pedigree.py`: ローカル環境でJSONからSVGを生成するCLIツール（オプション）。
*   `JSON_SCHEMA.md`: JSON中間表現の完全仕様 (v0.5)。
*   `開発ログ/`: 開発経緯の記録。
...
生成されたSVGファイルをPowerPointで編集する際のTips：

1. **SVGの貼り付け**: PowerPointにSVGファイルをドラッグ&ドロップまたは挿入
2. **図形として編集**: 画像を選択し、リボンメニューの「グラフィックス形式」から「**図形に変換**」を選択
3. **個別編集**: 変換後、各線・記号・文字を個別に選択・移動・削除・色変更が可能になります

> **💡 ヒント**: 図形に変換することで、線の位置調整や不要な要素の削除など、細かな調整が簡単に行えます。

## 主な機能と制約 (v0.5)

### v0.5 新機能（2025-12-13）⭐最新
*   ✅ **個人番号の視覚的順序表示**（JOHBOC図5完全準拠）: 記号の右上にアラビア数字のみを表示、左から右に1,2,3...と順番に付与（JSONの順番ではなく配置順、例：1, 2, 3）
*   ✅ **兄弟間横線の位置最適化**: 親の診断情報と重ならないよう、75%比率で下方に配置
*   ✅ **発端者マーカーの改善**: 45度の短縮矢印、P位置を矢印開始点下に配置し重なりを解消
*   ✅ **凡例機能の追加**: 複雑な家系図用に凡例を表示可能（meta.show_legend: trueで有効化）

### v0.4 新機能（2025-12-13）
*   ✅ **兄弟関係の描画**（親がいない兄弟）: 母方伯父・叔父など、親が家系図に含まれない兄弟の線描画に対応
*   ✅ **標準ID命名規則の明確化**: `I-1`, `I-2`, `II-1`形式を必須化（`I-2-brother`や`II-1-spouse`などの記述は不可）
*   ✅ **レイアウト改善**: 配偶者間隔・個体間隔を拡大し、文字の重なりを軽減
*   ✅ **GPTs出力フロー改善**: SVGダウンロードリンク → JSON表示 → 修正確認のルーチン化
...
*   ✅ 遺伝学的検査情報、双生児、養子、生殖補助技術（JSON v0.4スキーマで対応）
