...
- **v0.7** (2025-12-14): 図2（複数疾患）塗り分け・凡例制御・表記統一
  - **疾患別塗り分け（図2）**: `meta.legend_conditions` で疾患別に塗り分け、同一人物の複数疾患は左右分割で表現
  - **凡例制御の分離**: `meta.show_legend`（汎用Legend）と `meta.show_conditions_legend`（Conditionsのみ）を分離
  - **medical_notesの年齢表記統一**: `37歳/37才` 等を `37y` に正規化して表示
- **v0.6** (2025-12-14): 遺伝学的検査結果の描画と関係表現の明確化
  - **遺伝学的検査結果の描画**: `individuals[].genetic_testing.result`（または `display`）を個体記号の下に表示
  - **兄弟関係（親なし）と親子関係の使い分け**: `siblings` は親が家系図に含まれない兄弟の例外表現であり、親がいる場合は `partners` + `children`（夫婦または片親）で表現
...
現在の決定論的レンダラ（`pedigree_drawer_lib.PedigreeChart` v0.5）は、スキーマのうち「描画に必要な最小サブセット」を必ず解釈してSVGへ反映します。臨床情報として有用なフィールド（`diagnoses`/`medical_notes`/`genetic_testing`等）は保持でき、**診断情報・既往歴・遺伝学的検査結果も個体記号の下に描画**されます。**v0.4では兄弟関係（親なし）の描画**、**v0.5では個人番号の右上表示と凡例機能にも対応**しています。

レンダラが描画に使用する主なフィールド：
- `individuals[].gender`, `individuals[].sex_at_birth`
- `individuals[].status`
- 年齢系: `age`（v0.1互換）/ `current_age` / `age_at_death` / `birth_year` / `death_year`
- 遺伝学的検査: `individuals[].genetic_testing.result`（検査実施/結果がある場合）
...
```json
{
  "genetic_testing": {
    ...
    "display": "BRCA1 病的変異（+）",       // 描画用の短い表記（任意、指定があれば優先）
