...
現在の決定論的レンダラ（`pedigree_drawer_lib.PedigreeChart` v0.3）は、スキーマのうち「描画に必要な最小サブセット」を必ず解釈してSVGへ反映します。臨床情報として有用なフィールド（`diagnoses`/`genetic_testing`等）は保持できますが、**v0.3以降は診断情報と既往歴も描画に反映されます**。
...
| 診断時年齢 | ❌ 記録不可 | ✅ 記録可 | ✅ **描画対応** |
| 疾患名 | ❌ 記録不可 | ✅ 記録可 | ✅ **描画対応** |
| 複数疾患 | ❌ 不可 | ✅ `diagnoses[]` 配列 | ✅ 全て描画 |
| 年齢単位サフィックス | ❌ 不可 | ⚠️ 手動のみ | ✅ **自動付与** |
| 既往歴・手術歴 | ❌ 不可 | ❌ 不可 | ✅ **`medical_notes[]`** |
| 遺伝学的検査 | ❌ 不可 | ✅ `genetic_testing` | ✅ 継承 |
| 双生児 | ⚠️ 不完全 | ✅ `twin_info` | ✅ 継承 |
| 養子 | ❌ 不可 | ✅ `adoption_info` | ✅ 継承 |
| 生殖補助技術 | ❌ 不可 | ✅ `art_info` | ✅ 継承 |
| 近親婚詳細 | ❌ 不可 | ✅ `consanguinity` | ✅ 継承 |
| 複数個体の一括表記 | ❌ 不可 | ✅ `count` / `count_type` | ✅ 継承 |
| 右下の署名 | ⚠️ 作成者名含む | ⚠️ 作成者名含む | ✅ **日付のみ** |
