...
### 命名の手順
1. **発端者（Proband）**: 通常、自分の世代の真ん中あたりに配置（例: `II-2`）
2. **同世代の同胞**: 左から順に `II-1`, `II-2`, `II-3` など
3. **配偶者**: 発端者の配偶者も標準形式で（例: `II-3`）
4. **親世代**: 左から順に `I-1`, `I-2` など
5. **子世代**: 左から順に `III-1`, `III-2` など
...
5. **修正対応**: 修正点があれば、JSONを更新してSVGを再生成し、再度ダウンロードリンクとJSONを提示する
